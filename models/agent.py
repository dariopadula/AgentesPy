...
#### Otra version del agente (en un minuto puede recor más de una arista)

class AgenteMovil(Agent):
    VELOCIDADES = {
        "auto": 20_000 / 60,  # 50 km/h -> metros por minuto
        "bus": 15_000 / 60,   # 30 km/h
        "moto": 25_000 / 60,   # 30 km/h
        "bicicleta": 15_000 / 60,  # 15 km/h
        "caminata": 5_000 / 60  # 5 km/h
    }
    ...
    def step(self):
        ...
        if self.en_movimiento and self.ruta:
            tiempo_disponible = 1  # Minutos disponibles en este step

            while tiempo_disponible > 0 and len(self.ruta) > 1:
                if self.tiempo_restante_arista <= 0:  # Si no hay una arista en progreso
                    nodo_actual = self.ruta.pop(0)
                    nodo_siguiente = self.ruta[0]
                    distancia_arista = self.model.grafo[nodo_actual][nodo_siguiente][0]["length"]
                ...
                if self.tiempo_restante_arista > tiempo_disponible:
                    self.tiempo_restante_arista -= tiempo_disponible
                    tiempo_disponible = 0  # Se agota el tiempo en este step
                else:
                    tiempo_disponible -= self.tiempo_restante_arista
                    self.tiempo_restante_arista = 0
                    self.posicion_actual = self.ruta[0]  # Mueve al nodo siguiente

            # Si el agente llegó al destino
            if not self.ruta[1:]:
                self.en_movimiento = False
                self.model.schedule.remove(self)
                self.model.agentes_completados += 1
